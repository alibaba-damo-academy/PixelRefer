...
if [ ! -f "$output_file" ]; then
    for IDX in $(seq 0 $((CHUNKS-1))); do
        ...
        TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=${gpu_devices} python3 videorefer/eval/inference_video_mcqa_videomme.py \
            --model-path ${CKPT} \
...
python videorefer/eval/eval_video_mcqa_videomme.py \
