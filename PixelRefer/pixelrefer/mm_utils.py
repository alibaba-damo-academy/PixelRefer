...
def resize_image_mask(images, masks, mask_ids, patch_size=14, max_tokens=32):
    resize_images = []
    resize_masks = []
    mask_nums = []
    box_params = []
    for i, mask in enumerate(masks):
        ...
        scale_rate = math.ceil(math.sqrt(196*max_tokens/mask.sum()))
        if scale_rate==1:
            if (mask.sum()/196)>100:
        ...
        mask_nums.append(min(max_tokens, int(resize_mask.sum())))
