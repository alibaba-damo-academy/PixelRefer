...
from .pixelrefer_qwen2 import PixelreferQwen2ForCausalLM, PixelreferQwen2Config


VLLMs = {
    "pixelrefer_qwen2": PixelreferQwen2ForCausalLM,
}

VLLMConfigs = {
    "pixelrefer_qwen2": PixelreferQwen2Config,
}
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", **kwargs):
    ...
    # judge model type
    model_type = config.model_type if hasattr(config, "model_type") else kwargs.pop('model_type', "pixelrefer_qwen2")
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        if 'qwen2' in model_base.lower():
            model = PixelreferQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = PixelreferQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        if model_type in ['pixelrefer', 'pixelrefer_qwen2']:
            model = PixelreferQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = PixelreferQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        if model_type in ['pixelrefer_qwen2']:
            model = PixelreferQwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = PixelreferQwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
    else:
