...
class PixelreferMetaModel:

    def __init__(self, config):
        super(PixelreferMetaModel, self).__init__(config)
...
class PixelreferMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, position_ids=None, masks=None, additional_images = None, box_params=None,
    ):
        if self.config.use_token_compression:
            return self.prepare_inputs_labels_for_multimodal_with_compression(input_ids, attention_mask, past_key_values, labels, images, position_ids, masks, additional_images, box_params)
    ...
    def prepare_inputs_labels_for_multimodal_with_compression(
        ...
        if mask_selected.sum()>0:
            ...
            mask_feats = self.model.region_encoder(reshaped_features, masks, box_params, self.config.mask_num)
