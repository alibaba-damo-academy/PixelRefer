...
from pixelrefer.constants import (IGNORE_INDEX, MODAL_INDEX_MAP,
...
from pixelrefer.mm_utils import (load_images, load_video, load_video_from_ids,
                                  tokenizer_multimodal_token, annToMask, resize_image_mask)
from pixelrefer.model import *
from pixelrefer.pixelrefer_trainer import (
    PixelReferTrainer, find_all_linear_names, get_peft_state_maybe_zero_3,
    get_peft_state_non_lora_maybe_zero_3, safe_save_model_for_hf_trainer)
from pixelrefer.model.processor import PixelreferProcessor
...
class ModelArguments:
    # LLM Arguments
    model_type: Optional[str] = field(default="pixelrefer", metadata={"help": "Model type selected in the list: " + ", ".join(VLLMs.keys())})
    ...
    def _convert_normal(self, data_dict):
        ...
        if 'image' in data_dict and data_dict['image'] is not None:
            ...
            if isinstance(video_file, list) and len(video_file) == 1:
                ...
                images, timestamps, additional_frames = load_video_from_ids(video_file, fps=self.data_args.fps, max_frames=self.data_args.max_frames, frame_ids=frame_ids)
        ...
        if masks is not None and len(masks)>0:
            additional_frames, masks, mask_nums, box_params = resize_image_mask(additional_frames, masks, mask_ids, max_tokens=self.data_args.mask_num)
            conv_i = 0
        ...
        return modal, images, messages, image_downsampling, masks, additional_frames, box_params
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            if "stream" in data_dict and data_dict["stream"]:
                ...
                modal, images, messages, image_downsampling, masks, additional_frames, box_params = self._convert_normal(data_dict)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_encoder is not None:
        ...
        vlprocessor = PixelreferProcessor(vision_encoder.image_processor, tokenizer)
    ...
    # select a Trainer
    trainer = PixelReferTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
