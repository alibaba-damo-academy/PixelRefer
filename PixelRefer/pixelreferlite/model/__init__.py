...
from .pixelreferlite_qwen2 import PixelreferliteQwen2ForCausalLM, PixelreferliteQwen2Config


VLLMs = {
    "pixelreferlite_qwen2": PixelreferliteQwen2ForCausalLM,
}

VLLMConfigs = {
    "pixelreferlite_qwen2": PixelreferliteQwen2Config,
}
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", **kwargs):
    ...
    # judge model type
    model_type = config.model_type if hasattr(config, "model_type") else kwargs.pop('model_type', "pixelreferlite_qwen2")
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        if 'qwen2' in model_base.lower():
            model = PixelreferliteQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = PixelreferliteQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        if model_type in ['pixelreferlite', 'pixelreferlite_qwen2']:
            model = PixelreferliteQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = PixelreferliteQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        model.load_state_dict(mm_projector_weights, strict=False)
    elif 'pixelrefer' in model_type:
        # NOTE: SFT model loading
        tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, token=token)

        # model = PixelreferliteQwen2SlowFasetForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)

        # if model_type in ['pixelreferlite_qwen2']:
        #     model = PixelreferliteQwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
        # else:
        model = PixelreferliteQwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
    else:
