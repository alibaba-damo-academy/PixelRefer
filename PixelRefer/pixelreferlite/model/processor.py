...
"""
Processor class for PixelReferLite.
"""
import copy
import math
import warnings
...
import sys
sys.path.append(".")
from pixelreferlite.constants import DEFAULT_IMAGE_TOKEN, IGNORE_INDEX, REGION_TOKEN
...
class PixelreferliteProcessorKwargs(ProcessingKwargs, total=False):
    _defaults = {
        "text_kwargs": {
            "padding": False,
        },
...
class PixelreferliteProcessor(ProcessorMixin):
    r"""
    ...
    def __init__(self, image_processor=None, tokenizer=None, chat_template=None, **kwargs):
        ...
        self.region_token_id = self.tokenizer.convert_tokens_to_ids(REGION_TOKEN)
        self.eos_token_id = self.tokenizer.eos_token_id
        self.user_prompt = self._infer_user_prompt()
        self.user_prompt_ids = self.tokenizer.encode(self.user_prompt, return_tensors="pt")
        self.user_prompt_length = len(self.user_prompt_ids[0])
    ...
    def _process_text_with_label(
        ...
        **kwargs,
        ...
        for message_idx, message in enumerate(text):
            ...
            new_token_types = torch.full_like(input_ids, 3) # 0: image 1: text  2:region  3:ignore   4:system
            if message["role"] == "user":
                new_token_types[self.user_prompt_length:-1] = 1
                new_token_types[:self.user_prompt_length] = 4
                new_token_types[input_ids==self.image_token_id] = 0
                new_token_types[input_ids==self.region_token_id] = 2
            ...
            new_token_types_list.append(new_token_types)
        ...
        new_token_types = torch.cat(new_token_types_list)
    ...
    def _process_text_without_label(
        ...
        **kwargs,
        ...
        new_token_types[input_ids==self.image_token_id] = 0
        new_token_types[input_ids==self.region_token_id] = 2
        text_inputs["new_token_types"] = new_token_types
    ...
    def __call__(
        ...
        image_downsampling: Optional[int] = None,
        return_labels: bool = False,
        **kwargs: Unpack[PixelreferliteProcessorKwargs],
    ) -> BatchFeature:
        """
