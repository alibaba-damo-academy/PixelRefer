...
from .constants import NUM_FRAMES, MAX_FRAMES, NUM_FRAMES_PER_SECOND, MODAL_INDEX_MAP, DEFAULT_IMAGE_TOKEN, REGION_TOKEN
from pycocotools import mask as maskUtils

def generate_region_timestamp(timestamps, mask_ids, mask_nums, start_id):
    time_str = ''
    cur_id = start_id
    for mask_id in mask_ids:
        timestamp = timestamps[mask_id]
        time_str+=f'Time {round(timestamp,1)}:'+REGION_TOKEN*mask_nums[cur_id]+','
        cur_id+=1
    return '['+time_str[:-1]+']'
    
def _to_numpy_uint8(x):
    """把输入转换成 numpy.uint8 格式"""
...
def resize_image_mask(
    images,          # list/array of HxWxC (uint8) BGR/RGB 均可，cv2 只做插值
    masks,           # list/array of HxW 的{0,1}或bool
    mask_ids,        # 与 masks 对齐：每个掩码对应的 image 索引
    patch_size: int = 14,
    expanding_rate: int = 2,
    mask_token_num: int = 32,
):
    resize_images = []
    resize_masks = []
    raw_images = []
    ...
    # 缓存：同一张原图可能对应多个 mask
    img_resize_cache = {}   # id -> (raw_336, local_224)
    img_shape_cache = {}    # id -> (h, w)

    for i, mask in enumerate(masks):
        img_id = mask_ids[i]
        image = images[img_id]
        if img_id not in img_shape_cache:
            h, w = image.shape[:2]
            img_shape_cache[img_id] = (h, w)
        else:
            h, w = img_shape_cache[img_id]

        # --- 计算 mask 外接矩形（更快） ---
        m_u8 = _to_numpy_uint8(mask)

        pts = cv2.findNonZero(m_u8)
        if pts is None:
            # 空掩码：返回占位
            resize_images.append(np.zeros((patch_size, patch_size, image.shape[2]), dtype=image.dtype))
            resize_masks.append(torch.zeros((patch_size // patch_size, patch_size // patch_size), dtype=torch.float32))
            # 缓存并附加缩略图
            if img_id not in img_resize_cache:
                raw_336 = cv2.resize(image, (336, 336), interpolation=cv2.INTER_AREA)
                local_224 = cv2.resize(image, (224, 224), interpolation=cv2.INTER_AREA)
                img_resize_cache[img_id] = (raw_336, local_224)
            raw_images.append(img_resize_cache[img_id][0])
            local_images.append(img_resize_cache[img_id][1])
            # 空 bbox 与计数
            box_params.append(((0, 0, 0, 0), h, w))
            mask_nums.append(mask_token_num)
            continue

        # x,y 为左上角，bw,bh 为宽高（右开切片要用 +x,+y 到 +x+bw,+y+bh）
        x, y, bw, bh = cv2.boundingRect(pts)
        pad = patch_size * expanding_rate

        # 左上（含）与右下（不含）的裁剪坐标
        top = max(0, y - pad)
        left = max(0, x - pad)
        bottom_excl = min(h, y + bh + pad)
        right_excl  = min(w, x + bw + pad)

        # 维持对外兼容：返回 bbox 仍为右闭（原实现风格）
        bbox_return = (
            top,
            left,
            min(h - 1, bottom_excl - 1),
            min(w - 1, right_excl  - 1),
        )

        mask_h = bottom_excl - top
        mask_w = right_excl - left

        # --- 裁剪 ---
        cropping_img = image[top:bottom_excl, left:right_excl, :]
        cropping_mask = m_u8[top:bottom_excl, left:right_excl]

        # --- 目标分辨率比例 ---
        msum = int(mask.sum()) if mask.dtype != np.uint8 else int(mask.sum(dtype=np.int64))
        # 防 0
        scale_rate = math.ceil(math.sqrt(196.0 * mask_token_num / max(msum, 1)))
        if scale_rate == 1:
            # 保持你原有的“大掩码放缩回退”逻辑
            if (msum / 196.0) > 50.0:
                sr = math.sqrt((msum / 196.0) / 50.0)
                scale_rate = 1.0 / sr  # < 1 的缩小比例

        # 目标尺寸（对齐到 patch_size 的整数倍）
        ...
        # --- 图像缩放（根据缩放方向选择插值以提速/提质） ---
        interp_img = cv2.INTER_LINEAR if (resize_h >= mask_h or resize_w >= mask_w) else cv2.INTER_AREA
        resize_img = cv2.resize(cropping_img, (resize_w, resize_h), interpolation=interp_img)

        # --- 掩码缩放到 patch 网格大小（等价于 F.interpolate 双线性） ---
        grid_h = resize_h // patch_size
        grid_w = resize_w // patch_size
        # 先到网格尺寸（双线性，float32），保持与原返回张量一致
        mask_small = cv2.resize(
            cropping_mask.astype(np.float32),
            (grid_w, grid_h),
            interpolation=cv2.INTER_LINEAR
        )
        resize_mask = torch.from_numpy(mask_small)  # 2D, float32，与原返回类型一致

        # 固定使用传入的 mask_token_num（与原逻辑一致）
        mask_nums.append(mask_token_num)
        ...
        if img_id not in img_resize_cache:
            # 面积缩小优先 INTER_AREA
            raw_336 = cv2.resize(image, (336, 336), interpolation=cv2.INTER_AREA)
            local_224 = cv2.resize(cropping_img, (224, 224), interpolation=cv2.INTER_AREA)
            img_resize_cache[img_id] = (raw_336, local_224)
        ...
        raw_images.append(img_resize_cache[img_id][0])
        local_images.append(img_resize_cache[img_id][1])
        box_params.append((bbox_return, h, w))

    return resize_images, resize_masks, raw_images, mask_nums, box_params, local_images
    
def get_image_split_length(grid_thws):
    start_idx=0
    splited_features = []
    img_start_pos = []
    img_end_pos = []
    for thw_group in grid_thws:
        for tensor_thw in thw_group:
            ...
            num_elements = H * W
            img_start_pos.append(start_idx)
            start_idx += num_elements
            img_end_pos.append(start_idx)
    return img_start_pos, img_end_pos


def split_images(mm_features_raw, grid_thws):
    start_idx=0
    splited_features = []
    for thw_group in grid_thws:
        for tensor_thw in thw_group:
            ...
            split_tensor = mm_features_raw[start_idx:start_idx + num_elements]
            splited_features.append(split_tensor)
...
def load_video_from_specific_ids(video_path, frame_ids=None):
    ...
    sampled_frame_indices = frame_ids
    ...
    return frames, timestamps, frames
