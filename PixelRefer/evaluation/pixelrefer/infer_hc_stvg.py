...
import sys
sys.path.append('./')
from pixelrefer import disable_torch_init, model_init, mm_infer
...
from pixelrefer.mm_utils import load_video
...
class HC_STVG(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        question = "Please describe the object <region> in brief in the video."
...
def run_inference(args):
    ...
    final_data = []
    for i, (video_names, video, masks_, questions, mask_ids, answers) in enumerate(tqdm(val_loader)):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--video-folder', help='Directory containing video files.', default='data/HC-STVG/video_parts')
    parser.add_argument('--question-file', help='Path to the ground truth file containing question.', default='benchmarks/hc_stvg_val.json')
    parser.add_argument('--output-file', help='Directory to save the model results JSON.', default='./eval/hc_stvg/stage1-output.json')
